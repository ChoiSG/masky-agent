...
namespace Masky
{
    ...
    public class Cert
    {
        ...
        {
            if (length < 0) throw new ArgumentOutOfRangeException("length", "bad length negative");
            if (length < 0x80)
            {
                // Short form
        ...
        public static IX509PrivateKey CreatePrivateKey(bool machineContext)
        {
        ...
        public CertificateRequest CreateCertRequestMessage()
        {
            string subjectName = "";
        ...
        {
            if (csp.PublicOnly) throw new ArgumentException("No private key inside csp sadge", "csp");
        ...
        public void GetCertUser()
        {
            ...
            if (certPemString != "")
            {
                ...
                Console.WriteLine("[+] Cert User: '{0}'", UserName);
            }
            else if (certPemString == "" && csr.PrivateKeyPem != "")
            {
                Console.Error.WriteLine("No cert for this dude: '{0}'", UserName);
            }
            return;
        }
    }
