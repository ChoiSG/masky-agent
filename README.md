### This is just Masky Agent code from the Masky project 
- [Masky by @Z4kSec](https://github.com/Z4kSec/Masky)

### Masky Agent only, for CI/CD purposes 
- Only contains Masky agent 
- Added /outputconsole for writing output to console 
- Added fody, costura fody for single application compilation 
- Just a localized version so I can use it for jenkins ci/cd for obfuscation with codeception/invisibilitycloak

### notes 
```
- Run Codeception
.\Codecepticon.exe --action obfuscate --module csharp --verbose --path "C:\opt\testoo\Masky.sln" --rename all --rename-method markov --markov-min-length 8 --markov-max-length 14 --markov-min-words 5 --markov-max-words 7 --string-rewrite --string-rewrite-method single

- Change Assembly name and namespace in project properties 
- Change `AssemblyInfo.cs` related   

.\Moosky.exe /ca:'ca-fqdn\ca-name' /outputconsole

- Beware of using ThreatCheck, it might start feeding to defender ml. 
```
